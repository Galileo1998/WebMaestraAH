...
try {
    ...
    $saveSnapshot = static function (PDO $connection, int $taskId, string $event): void {
        try {
            ...
            $last=$connection->prepare('SELECT id,estado_hash,evento,created_at FROM ah_monitoreo_historial WHERE id_poa=? ORDER BY id DESC LIMIT 1');$last->execute([$taskId]);$previous=$last->fetch(PDO::FETCH_ASSOC);if($previous&&hash_equals((string)$previous['estado_hash'],$hash))return;
            $period=(string)($task['operativo_periodo']??'');$user=(string)($_SESSION['email']??$_SESSION['user_email']??$_SESSION['nombre']??'V2');$recent=$previous&&$previous['evento']===$event&&(time()-strtotime((string)$previous['created_at']))<=300;if($recent){$update=$connection->prepare('UPDATE ah_monitoreo_historial SET periodo=?,estado_json=?,estado_hash=?,usuario=?,updated_at=CURRENT_TIMESTAMP WHERE id=?');$update->execute([$period,$json,$hash,$user,(int)$previous['id']]);}else{$insert=$connection->prepare('INSERT INTO ah_monitoreo_historial (id_poa,periodo,evento,estado_json,estado_hash,usuario) VALUES(?,?,?,?,?,?)');$insert->execute([$taskId,$period,$event,$json,$hash,$user]);}
    ...
    if ($action === 'copy_previous_month' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        $requireV2Csrf();if(empty($_SESSION['metas_edit_unlocked_until'])||(int)$_SESSION['metas_edit_unlocked_until']<time())throw new RuntimeException('La autorización para editar metas venció.');$taskId=(int)($_POST['id_poa']??0);$cycle=['jul','aug','sep','oct','nov','dec','jan','feb','mar','apr','may','jun'];$calendar=[1=>'jan',2=>'feb',3=>'mar',4=>'apr',5=>'may',6=>'jun',7=>'jul',8=>'aug',9=>'sep',10=>'oct',11=>'nov',12=>'dec'];$current=$calendar[(int)date('n')];$index=array_search($current,$cycle,true);if($index===false||$index===0)throw new RuntimeException('No hay un mes anterior disponible en el ciclo fiscal.');$previous=$cycle[$index-1];$db->beginTransaction();$stmt=$db->prepare("UPDATE ah_poa SET op_act_{$current}=op_act_{$previous},op_part_{$current}=op_part_{$previous},op_editado_{$current}=1 WHERE id=? AND is_active=1");$stmt->execute([$taskId]);$rows=$db->prepare('SELECT * FROM ah_poa_asignaciones WHERE id_poa=? FOR UPDATE');$rows->execute([$taskId]);$updated=[];foreach($rows->fetchAll(PDO::FETCH_ASSOC) as $assignment){$assignment['meta_'.$current]=(float)($assignment['meta_'.$previous]??0);$metaTotal=0.0;$logroTotal=0.0;$active=[];foreach($cycle as $month){$metaTotal+=(float)($assignment['meta_'.$month]??0);$logroTotal+=(float)($assignment['logro_'.$month]??0);if((float)($assignment['meta_'.$month]??0)>0||(float)($assignment['logro_'.$month]??0)>0)$active[]=$month;}$update=$db->prepare("UPDATE ah_poa_asignaciones SET meta_{$current}=?,meta_asignada=?,logro_asignado=?,meses_asignados=? WHERE id=? AND id_poa=?");$update->execute([$assignment['meta_'.$current],$metaTotal,$logroTotal,implode(', ',$active),(int)$assignment['id'],$taskId]);$updated[(string)$assignment['id']]=['value'=>$assignment['meta_'.$current],'meta_total'=>$metaTotal,'logro_total'=>$logroTotal];}$db->commit();$taskStmt=$db->prepare("SELECT op_act_{$current},op_part_{$current} FROM ah_poa WHERE id=?");$taskStmt->execute([$taskId]);$goals=$taskStmt->fetch(PDO::FETCH_ASSOC)?:[];$saveSnapshot($db,$taskId,'autollenar_mes_v2');echo json_encode(['status'=>'ok','current'=>$current,'previous'=>$previous,'op_act'=>(float)($goals['op_act_'.$current]??0),'op_part'=>(float)($goals['op_part_'.$current]??0),'assignments'=>$updated],JSON_UNESCAPED_UNICODE);exit;
    }
    ...
    if ($action === 'save_stage_meta' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        $requireV2Csrf();$taskId=(int)($_POST['id_poa']??0);$stageId=(int)($_POST['stage_id']??0);$responsibles=json_decode((string)($_POST['responsables']??'[]'),true);$units=json_decode((string)($_POST['unidades']??'[]'),true);if(!is_array($responsibles))$responsibles=[];if(!is_array($units))$units=[];$responsibles=array_values(array_unique(array_filter(array_map('strval',$responsibles))));$units=array_values(array_unique(array_filter(array_map('strval',$units))));$date=trim((string)($_POST['fecha_recepcion']??''));if($date!==''&&!preg_match('/^\d{4}-\d{2}-\d{2}$/',$date))throw new RuntimeException('Fecha no válida.');
        $db->beginTransaction();$stmt=$db->prepare('SELECT involucrados_json FROM ah_poa_etapas WHERE id=? AND id_poa=? LIMIT 1 FOR UPDATE');$stmt->execute([$stageId,$taskId]);$raw=$stmt->fetchColumn();if($raw===false)throw new RuntimeException('Etapa no encontrada.');$rows=json_decode((string)$raw,true);if(!is_array($rows))$rows=[];$baseStmt=$db->prepare("SELECT DISTINCT base_asignada FROM ah_poa_asignaciones WHERE id_poa=? AND tecnico=? AND COALESCE(base_asignada,'')<>'' ORDER BY base_asignada");foreach($responsibles as $person){$baseStmt->execute([$taskId,$person]);$bases=$baseStmt->fetchAll(PDO::FETCH_COLUMN);foreach($units as $unit){$key=rtrim(base64_encode($person.'|__ALLBASES|'.$unit),'=');if(isset($rows[$key])&&is_array($rows[$key]))continue;$rows[$key]=['persona'=>$person,'unidad'=>$unit,'base'=>implode('|',$bases),'mes'=>'','a_lograr'=>0,'cumplido'=>0,'a_tiempo'=>100,'en_forma'=>100,'verifics'=>[],'lugar'=>[],'centros'=>[],'deleted'=>false,'quality_initialized'=>true,'quality_version'=>2];}}
        $update=$db->prepare('UPDATE ah_poa_etapas SET responsable=?,unidad_medida=?,fecha_recepcion=?,involucrados_json=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND id_poa=?');$update->execute([json_encode($responsibles,JSON_UNESCAPED_UNICODE),json_encode($units,JSON_UNESCAPED_UNICODE),$date!==''?$date:null,json_encode($rows,JSON_UNESCAPED_UNICODE),$stageId,$taskId]);$db->commit();$saveSnapshot($db,$taskId,'configuracion_etapa_v2');echo json_encode(['status'=>'ok','responsables'=>$responsibles,'unidades'=>$units,'fecha_recepcion'=>$date,'rows'=>$rows],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);exit;
    }
    if ($action === 'ensure_default_stages' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        $requireV2Csrf();$taskId=(int)($_POST['id_poa']??0);if($taskId<1)throw new RuntimeException('Actividad no válida.');$defaults=[['E-1','Diseñar Actividad','Diseñar, facilitar y explicar a ET e implementadores lineamientos, recursos, protocolos y guías.',3],['E-2','Organizar y socializar Actividad','Organizar y facilitar la logística antes, durante y después de la actividad. Listados, actas, alimentación, transporte, etc.',6],['E-3','Desarrollar y reportar Actividad','Desarrollar actividad o evento en base a criterios de calidad y objetivos establecidos.',20],['E-4','Asistencia y Monitoreo de Actividad','Acompañar la actividad aplicando herramientas evaluativas a la calidad del proceso y el nivel de satisfacción de participantes.','last']];$db->beginTransaction();$exists=$db->prepare('SELECT COUNT(*) FROM ah_poa_etapas WHERE id_poa=?');$exists->execute([$taskId]);if((int)$exists->fetchColumn()===0){$insert=$db->prepare("INSERT INTO ah_poa_etapas (id_poa,codigo_etapa,nombre_etapa,descripcion_etapa,unidad_medida,responsable,involucrados_json,fecha_recepcion,orden) VALUES(?,?,?,?,? ,?,?,?,?)");$year=(int)date('Y');$month=(int)date('n');$last=(int)date('t');foreach($defaults as $index=>$item){$day=$item[3]==='last'?$last:(int)$item[3];$date=sprintf('%04d-%02d-%02d',$year,$month,min($day,$last));$insert->execute([$taskId,$item[0],$item[1],$item[2],'[]','[]','{}',$date,$index+1]);}}$db->commit();$stmt=$db->prepare('SELECT * FROM ah_poa_etapas WHERE id_poa=? ORDER BY orden,id');$stmt->execute([$taskId]);$stages=$stmt->fetchAll(PDO::FETCH_ASSOC);$saveSnapshot($db,$taskId,'inicializar_etapas_v2');echo json_encode(['status'=>'ok','etapas'=>$stages],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);exit;
    }
