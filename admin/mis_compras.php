...
if(!function_exists('normalizedOwnerTokens')){
...
if(!function_exists('historicalPurchaseMatchesUser')){
...
if(!function_exists('truncate_text')){
    function truncate_text(string $text, int $length = 70): string {
        if (function_exists('mb_strimwidth')) return mb_strimwidth($text, 0, $length, '…');
        return strlen($text) > $length ? substr($text, 0, $length) . '…' : $text;
    }
}

// Extracción de credenciales robusta para cualquier sistema de sesiones
$isAdmin = (isset($_SESSION['rol']) && $_SESSION['rol'] === 'Administrador') || (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin');
$userId = (int)($_SESSION['user_id'] ?? $_SESSION['usuario_id'] ?? $_SESSION['id_usuario'] ?? $_SESSION['id'] ?? ($_SESSION['user']['id'] ?? 0));
...
try {
    ...
    // Lo aislamos en su propio try-catch por si la tabla "users" usa "nombre" en vez de "name"
    try {
    ...
    if ($userId > 0) {
        ...
        // Si no encontró "name", intenta con "nombre"
        if (!$userName) {
            try {
                $userStmt2=$db->prepare('SELECT nombre FROM users WHERE id=? LIMIT 1');
                $userStmt2->execute([$userId]);
                $userName=(string)$userStmt2->fetchColumn();
            } catch(Throwable $e) {}
        }
...
try {
    ...
    error_log('mis_compras: ' . $e->getMessage());
    $error_db = 'No fue posible consultar las compras en este momento.';
}

$months = [];
try {
    $months=$db->query("SELECT DISTINCT DATE_FORMAT(fecha,'%Y-%m') mes FROM ah_compras WHERE fecha IS NOT NULL ORDER BY mes DESC")->fetchAll(PDO::FETCH_COLUMN);
} catch(Throwable $e) {}
...
<main class="main">
  ...
  <?php if($msg): ?>
      <div class="notice <?= $error_db !== '' ? 'error' : '' ?>">
          <i class="fa-solid <?= $error_db !== '' ? 'fa-triangle-exclamation' : 'fa-circle-check' ?>"></i> <?=h($msg)?>
      </div>
  <?php endif; ?>
  ...
  <?php if($compras): foreach($compras as $c): ?>
    ...
    <td><?=h($c['solicitante'])?></td><td><?=h(truncate_text((string)$c['descripcion_actividad'], 70))?></td><td><?=h($c['marco_logico']?:'—')?></td><td><strong>L. <?=number_format((float)$c['monto_total'],2)?></strong></td><td><?=statusBadge((string)$c['estado'])?></td>
